...
} = require("../validation/PacienteValidation");
    ...
    if (paciente) {
      ...
      if (hepatiteb) {
        ...
        if (pacienteHepatiteB) {
          ...
          try {
            await hepatiteRequiredScheme.validate(hepatiteb, {
              abortEarly: false
            }); // AbortEarly para fazer todas as validações
          } catch (error) {
            throw new AppError(error.name, 422, error.errors);
          }
          await PacienteHepB.create({
        ...
        await Promise.all(
          hepatitec.map(async hepc => {
            const alreadyExists = registrosHepC.some(
              registroHC => hepc.id == registroHC.id
            );
            ...
            if (alreadyExists) {
              ...
              // Revalida dados, pois para criação há atributos obrigatórios
              try {
                await hepatiteRequiredScheme.validate(hepc, {
                  abortEarly: false
                }); // AbortEarly para fazer todas as validações
              } catch (error) {
                throw new AppError(error.name, 422, error.errors);
        ...
        await Promise.all(
          registrosHepC.map(async hepc => {
            if (
              !registrosIdsAtualizadosHepC.some(
                registrohc => hepc.id == registrohc
              )
            )
